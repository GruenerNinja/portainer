...
func (h *Handler) getSource(w http.ResponseWriter, r *http.Request) *httperror.HandlerError {
	...
	detail := BuildSourceDetail(h.buildSource(r.Context(), source, stats), source, sourceWfs, access)
	return response.JSON(w, detail)
}

func BuildSourceDetail(baseSource Source, src *portainer.Source, sourceWfs []workflows.Workflow, access SourceAccess) SourceDetail {
